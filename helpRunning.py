...
#     if (lineCount(file[i]) > 2) # kill open processes:
			...
			return done
	cur_cpu = []
	for i_cpu in range(5):
		cur_cpu.append(get_cpu_load())
		time.sleep(2)
	cur_cpu_aver = sum(cur_cpu)/5
	if cur_cpu_aver < inAnT.cpu_percentage_kill:
		done = 1
		kill_hfss(i)
		print('Hfss has been killed --- error detected in HFSS LIBARAY, cpu_percentage: ', cur_cpu)
		time.sleep(10)
	#	return done
	return done
#####################################################################################################################3
def assignFitness(nameDir,gen):
	...
	if not file.is_file():
		...
		for i in range(len(Band)):
			...
			if Band[i] < -10:
				fitness = fitness + Band[i] - inAnT.S11_penalty*(Band[i]/-10)**2 # penalty the point s11 << -10
			else:
		...
		if signal_stop_i and signal_start_i:
			...
			fitness = fitness - inAnT.bandwidth_penalty*bandwidth_1					 # bandwidth 
			print(inAnT.bandwidth_penalty*bandwidth_1)
		...
		if inAnT.band2_examination:
			...
			for i in range(len(Band_2)):									###### new
				if Band_2[i] < -10:
					fitness = fitness + Band_2[i] - inAnT.S11_penalty*(Band_2[i]/-10)**2
				else:
			...
			if signal_stop_i and signal_start_i:
				...
				fitness = fitness - inAnT.bandwidth_penalty*bandwidth_2
				print(inAnT.bandwidth_penalty*bandwidth_2)
		...
		if inAnT.band2_examination:
			...
			exist_band2 = False
			if ((min_S11_2 in so_best_2) and (min_S11_2 < -10)):
				fitness = fitness - inAnT.resonant_penalty
				exist_band2 = True

		exist1 = False
		exist2 = False
		if ((min_S11_1 in so_best_1) and (min_S11_1 < -10)):
		...
		if inAnT.band2_examination:
			if exist1 and exist_band2:
				fitness = fitness - inAnT.resonant_penalty_two_band

		if inAnT.band2_examination:
			if (min(Band) > -10) and (min(Band_2) > -10) and (min(S11) < -10): ########## FITNESS for the resonant point that is not 
				fitness = sum(S11)												# in the desired band.
		else:
			if (min(Band) > -10) and (min(S11) < -10): ########## FITNESS for the resonant point that is not 
				fitness = sum(S11)	
